...
const { OperationNames, LockState, LockStateNames } = require('./Enums');
...
class Monitor {
  ...
  */
  ...
  async handleOperationMonitor() {
    ...
    await this.errorReset(`${OperationNames[operation.type]} operation failed`, 'errors.response');
  }
  ...
  async handleStateMonitor() {
    ...
    const { state } = data.lockProperties;
