...
class Device extends OAuth2Device {
  ...
  */
  ...
  async setCapabilities(deviceData) {
    ...
    if (deviceData.hasOwnProperty('softwareVersions')) {
      if (deviceData.softwareVersions[0]) {
  ...
  async _setStore(deviceData) {
    ...
    const settings = deviceData.deviceSettings;

    if (!settings.hasOwnProperty('pullSpringEnabled')) {
      return;
    }

    const pullSpringEnabled = settings.pullSpringEnabled ? 'on' : 'off';
  ...
  */
  ...
  async handleStateMonitor() {
    ...
    // Stop state monitor at 15 or more tries
    if (this.numberOfTries > 14) {
