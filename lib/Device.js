...
class Device extends OAuth2Device {
  ...
  */
  ...
  async onCapabilityLocked(lock) {
    try {
      const currentValue = await this.getCapabilityValue('locked');
  ...
  async onCapabilityOpen(open) {
    try {
      const currentValue = await this.getCapabilityValue('open');

      // Skip when old- and new values are the same, or open is false
