...
class Driver extends OAuth2Driver {
  ...
  */
  ...
  async onOAuth2Init() {
    ...
    this.log(`${this.name} driver initialized`);
  ...
  async onPairListDevices({oAuth2Client}) {
    ...
    availableDevices.forEach(device => {
      ...
      if (this.id === 'lock') {
        ...
        settings.pull_spring_enabled = device.deviceSettings.pullSpringEnabled ? 'on' : 'off';
      }
