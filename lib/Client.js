...
class Client extends OAuth2Client {
  ...
  */
  ...
  async getBridge(id) {
    ...
    this.log(`Bridge ${id} response:`, JSON.stringify(response));
  ...
  */
  ...
  async pullSpring(id) {
    this.log(`Sending "pull" command for lock ${id}`);

    const response = await this.post({
      path: `/my/lock/${id}/operation/pull`,
      query: '',
      json: null,
      body: null,
      headers: {},
    ...
    this.log(`Pull ${id} response:`, JSON.stringify(response));
  ...
  async updateLockSettings(id, settings) {
    ...
    const response = await this.patch({
      ...
      json: {
        id,
        revision: await this.getRevision(id),
