...
class Client extends OAuth2Client {
  ...
  */
  ...
  async syncBridge(tedeeId) {
    this.log(`Fetching bridge ${tedeeId} from API`);
    ...
    this.log(`Bridge ${tedeeId} API response:`, JSON.stringify(response));
  ...
  */
  ...
  async getLock(tedeeId) {
    this.log(`Fetching lock ${tedeeId} from API`);
    ...
    this.log(`Lock ${tedeeId} API response:`, JSON.stringify(response));
   ...
   */
  async close(tedeeId) {
    this.log(`Sending "close" lock ${tedeeId} to API`);

    const response = await this.post({
      path: '/my/lock/close',
      query: '',
      json: { deviceId: tedeeId },
      body: null,
      headers: {}
    });

    this.log(`Close ${tedeeId} API response:`, JSON.stringify(response));
  ...
  async pullSpring(tedeeId) {
    this.log(`Sending "pull-spring" for lock ${tedeeId} to API`);
    ...
    this.log(`Pull spring ${tedeeId} API response:`, JSON.stringify(response));
   ...
   */
  async open(tedeeId) {
    this.log(`Sending "open" lock ${tedeeId} to API`);
    ...
    this.log(`Open ${tedeeId} API response:`, JSON.stringify(response));
  ...
  async updateLockSettings(tedeeId, deviceSettings) {
    this.log(`Updating lock settings ${tedeeId}:`, JSON.stringify(deviceSettings));
    ...
    this.log(`Lock settings ${tedeeId} API response:`, JSON.stringify(response));
  }
